...
int main(int argc, char *argv[]) {
    ...
    /*获得网络接口配置参数*/
    {
        /*查询网卡"eth0"的情况*/
        struct ifreq ifr;
        memcpy(ifr.ifr_name, "eth0", 5);/*获取标记*/
        ...
        if (!err) {
            printf("SIOCGIFMAP, mem_start:%ld, mem_end:%ld, base_addr:%d, irq:%d, dma:%d, port: %d\n",
                   ifr.ifr_map.mem_start,/*开始地址*/ifr.ifr_map.mem_end,/*结束地址*/
                   ifr.ifr_map.base_addr/*基地址*/, ifr.ifr_map.irq/*中断*/, ifr.ifr_map.dma/*直接访问内存*/,
                   ifr.ifr_map.port/*端口*/);
        }
