...
int main(int argc, char *arqv[]) {
    ...
    memset(&addr_serv, 0, sizeof(addr_serv));/*填充服务器地址*/
    addr_serv.sin_family = AF_INET;/*清零*//*AF INET协议族*/
    addr_serv.sin_addr.s_addr = inet_addr("127.0.0.1");/*地址为127.0.0.1*//*服务器端口*/
    addr_serv.sin_port = htons(PORT_SERV);
    connect(s, (struct sockaddr *) &addr_serv, sizeof(addr_serv));/*连接服务器*/
    getsockname(s, (struct sockaddr *) &local, &len);/*获得套接字文件描述符的地址*/
    printf("UDP local addr:%s\n", inet_ntoa(local.sin_addr));/*打印获得的地址*/
    close(s);
    return 0;
}